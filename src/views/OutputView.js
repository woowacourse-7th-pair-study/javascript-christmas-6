...
const OutputView = {
  ...
  printResult(menus, totalPrice, benefitDiscount) {
    ...
    const totalBenefitDiscount = benefitDiscount.dday + benefitDiscount.weekday + benefitDiscount.weekend;
  ...
  printBenefitDiscount(benefitDiscount) {
    Console.print('<혜택 내역>');
    if (benefitDiscount.dday === 0 && benefitDiscount.weekday === 0 && benefitDiscount.weekend === 0) {
      Console.print('없음');
    }
    if (benefitDiscount.dday > 0) {
      Console.print(`크리스마스 디데이 할인: -${parser.priceToLocaleString(benefitDiscount.dday)}원`);
    }
    if (benefitDiscount.weekday > 0) {
      Console.print(`평일 할인: -${parser.priceToLocaleString(benefitDiscount.weekday)}원`);
    }
    if (benefitDiscount.weekend > 0) {
      Console.print(`주말 할인: -${parser.priceToLocaleString(benefitDiscount.weekend)}원`);
    }
  },
